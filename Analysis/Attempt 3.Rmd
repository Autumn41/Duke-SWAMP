...
SWAMP_means_medians<- ALL_SWAMP_data %>% 
    group_by(`Site`, `Year`, `Month`, `Day`, `date`) %>% 
...
#Create daily average mean of TN for: WT1, BR5, WT2, WT3, WT5
ALL_SWAMP_data$daymonth <- strftime(ALL_SWAMP_data$date, format="%m-%d") 
WT1 <- ALL_SWAMP_data %>% 
  filter(Site %in% c("WT1"))
WT1_dailymean<- 
  transform(WT1, TN =ave(`Total N (ug/L)`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `Total.N..ug.L.` =ifelse(is.na(`Total.N..ug.L.`),TN,`Total.N..ug.L.`)) %>% 
  transform(WT1, TSS =ave(`TSS..mg.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `TSS..mg.L.` =ifelse(is.na(`TSS..mg.L.`),TSS,`TSS..mg.L.`)) %>% 
  transform(WT1, pH =ave(`pH`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `pH` =ifelse(is.na(`pH`),pH,`pH`)) %>% 
  transform(WT1, UTP =ave(`UTP`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `UTP` =ifelse(is.na(`UTP`),UTP,`UTP`)) %>% 
  transform(WT1, FC =ave(`FC`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `FC` =ifelse(is.na(`FC`),FC,`FC`)) %>% 
  transform(WT1, NH =ave(`NH4.N..ug.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `NH4.N..ug.L.` =ifelse(is.na(`NH4.N..ug.L.`),NH,`NH4.N..ug.L.`)) %>% 
  transform(WT1, Temp =ave(`Temp.C`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT1, `Temp.C` =ifelse(is.na(`Temp.C`),Temp,`Temp.C`)) %>% 
...
WT1_dailymean <- WT1_dailymean[!duplicated(WT1_dailymean[c('date')]),]

BR5 <- ALL_SWAMP_data %>% 
  filter(Site %in% c("BR5"))
BR5_dailymean<- 
  transform(BR5, TN =ave(`Total N (ug/L)`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `Total.N..ug.L.` =ifelse(is.na(`Total.N..ug.L.`),TN,`Total.N..ug.L.`)) %>% 
  transform(BR5, TSS =ave(`TSS..mg.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `TSS..mg.L.` =ifelse(is.na(`TSS..mg.L.`),TSS,`TSS..mg.L.`)) %>% 
  transform(BR5, pH =ave(`pH`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `pH` =ifelse(is.na(`pH`),pH,`pH`)) %>% 
  transform(BR5, UTP =ave(`UTP`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `UTP` =ifelse(is.na(`UTP`),UTP,`UTP`)) %>% 
  transform(BR5, FC =ave(`FC`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `FC` =ifelse(is.na(`FC`),FC,`FC`)) %>% 
  transform(BR5, NH =ave(`NH4.N..ug.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `NH4.N..ug.L.` =ifelse(is.na(`NH4.N..ug.L.`),NH,`NH4.N..ug.L.`)) %>% 
  transform(BR5, Temp =ave(`Temp.C`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(BR5, `Temp.C` =ifelse(is.na(`Temp.C`),Temp,`Temp.C`)) %>% 
          select(`Site`, `date`, `TN`, `TSS`, `pH`, `UTP`, `FC`, `NH`, `Temp`) %>%     
  replace_with_na(replace = list(TN = "NaN")) %>% 
  replace_with_na(replace = list(TSS = "NaN")) %>% 
  replace_with_na(replace = list(pH = "NaN")) %>% 
  ...
  replace_with_na(replace = list(NH = "NaN")) %>% 
  replace_with_na(replace = list(Temp = "NaN")) 
BR5_dailymean <- BR5_dailymean[!duplicated(BR5_dailymean[c('date')]),]

WT2 <- ALL_SWAMP_data %>% 
  filter(Site %in% c("WT2"))
WT2_dailymean<-   
  transform(WT2, TN =ave(`Total N (ug/L)`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `Total.N..ug.L.` =ifelse(is.na(`Total.N..ug.L.`),TN,`Total.N..ug.L.`)) %>% 
  transform(WT2, TSS =ave(`TSS..mg.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `TSS..mg.L.` =ifelse(is.na(`TSS..mg.L.`),TSS,`TSS..mg.L.`)) %>% 
  transform(WT2, pH =ave(`pH`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `pH` =ifelse(is.na(`pH`),pH,`pH`)) %>% 
  transform(WT2, UTP =ave(`UTP`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `UTP` =ifelse(is.na(`UTP`),UTP,`UTP`)) %>% 
  transform(WT2, FC =ave(`FC`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `FC` =ifelse(is.na(`FC`),FC,`FC`)) %>% 
  transform(WT1, NH =ave(`NH4.N..ug.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `NH4.N..ug.L.` =ifelse(is.na(`NH4.N..ug.L.`),NH,`NH4.N..ug.L.`)) %>% 
  transform(WT1, Temp =ave(`Temp.C`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT2, `Temp.C` =ifelse(is.na(`Temp.C`),Temp,`Temp.C`)) %>% 
          select(`Site`, `date`, `TN`, `TSS`, `pH`, `UTP`, `FC`, `NH`, `Temp`) %>%     
  replace_with_na(replace = list(TN = "NaN")) %>% 
  replace_with_na(replace = list(TSS = "NaN")) %>% 
  replace_with_na(replace = list(pH = "NaN")) %>% 
  ...
  replace_with_na(replace = list(NH = "NaN")) %>% 
  replace_with_na(replace = list(Temp = "NaN")) 
WT2_dailymean <- WT2_dailymean[!duplicated(WT2_dailymean[c('date')]),]

WT3 <- ALL_SWAMP_data %>% 
  filter(Site %in% c("WT3"))
WT3_dailymean<-   
  transform(WT3, TN =ave(`Total N (ug/L)`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `Total.N..ug.L.` =ifelse(is.na(`Total.N..ug.L.`),TN,`Total.N..ug.L.`)) %>% 
  transform(WT3, TSS =ave(`TSS..mg.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `TSS..mg.L.` =ifelse(is.na(`TSS..mg.L.`),TSS,`TSS..mg.L.`)) %>% 
  transform(WT3, pH =ave(`pH`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `pH` =ifelse(is.na(`pH`),pH,`pH`)) %>% 
  transform(WT3, UTP =ave(`UTP`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `UTP` =ifelse(is.na(`UTP`),UTP,`UTP`)) %>% 
  transform(WT3, FC =ave(`FC`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `FC` =ifelse(is.na(`FC`),FC,`FC`)) %>% 
  transform(WT3, NH =ave(`NH4.N..ug.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `NH4.N..ug.L.` =ifelse(is.na(`NH4.N..ug.L.`),NH,`NH4.N..ug.L.`)) %>% 
  transform(WT3, Temp =ave(`Temp.C`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT3, `Temp.C` =ifelse(is.na(`Temp.C`),Temp,`Temp.C`)) %>% 
          select(`Site`, `date`, `TN`, `TSS`, `pH`, `UTP`, `FC`, `NH`, `Temp`) %>%     
  replace_with_na(replace = list(TN = "NaN")) %>% 
  replace_with_na(replace = list(TSS = "NaN")) %>% 
  replace_with_na(replace = list(pH = "NaN")) %>% 
  ...
  replace_with_na(replace = list(NH = "NaN")) %>% 
  replace_with_na(replace = list(Temp = "NaN")) 
WT3_dailymean <- WT3_dailymean[!duplicated(WT3_dailymean[c('date')]),]

WT5 <- ALL_SWAMP_data %>% 
  filter(Site %in% c("WT5"))
WT5_dailymean<-   
  transform(WT5, TN =ave(`Total N (ug/L)`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `Total.N..ug.L.` =ifelse(is.na(`Total.N..ug.L.`),TN,`Total.N..ug.L.`)) %>% 
  transform(WT5, TSS =ave(`TSS..mg.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `TSS..mg.L.` =ifelse(is.na(`TSS..mg.L.`),TSS,`TSS..mg.L.`)) %>% 
  transform(WT5, pH =ave(`pH`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `pH` =ifelse(is.na(`pH`),pH,`pH`)) %>% 
  transform(WT5, UTP =ave(`UTP`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `UTP` =ifelse(is.na(`UTP`),UTP,`UTP`)) %>% 
  transform(WT5, FC =ave(`FC`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `FC` =ifelse(is.na(`FC`),FC,`FC`)) %>% 
  transform(WT5, NH =ave(`NH4.N..ug.L.`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `NH4.N..ug.L.` =ifelse(is.na(`NH4.N..ug.L.`),NH,`NH4.N..ug.L.`)) %>% 
  transform(WT5, Temp =ave(`Temp.C`, daymonth, FUN=function(x) mean(x,na.rm=TRUE) )) %>% 
              transform(WT5, `Temp.C` =ifelse(is.na(`Temp.C`),Temp,`Temp.C`)) %>% 
          select(`Site`, `date`, `TN`, `TSS`, `pH`, `UTP`, `FC`, `NH`, `Temp`) %>%     
  replace_with_na(replace = list(TN = "NaN")) %>% 
  replace_with_na(replace = list(TSS = "NaN")) %>% 
  replace_with_na(replace = list(pH = "NaN")) %>% 
  ...
  replace_with_na(replace = list(NH = "NaN")) %>% 
  replace_with_na(replace = list(Temp = "NaN")) 
WT5_dailymean <- WT5_dailymean[!duplicated(WT5_dailymean[c('date')]),]
